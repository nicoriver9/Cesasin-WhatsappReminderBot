...
const AppointmentCancelledTable: React.FC<AppointmentCancelledTableProps> = ({ 
  appointments, 
  // refreshAppointments 
}) => {
  ...
  const [filteredAppointments, setFilteredAppointments] = useState<CancelledAppointment[]>([]);
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      results = results.filter(appointment => {
        ...
        if (searchField === 'date') {
          fieldValue = new Date(appointment.cancellation_date).toLocaleDateString('es-AR');
        } else {
    ...
    results.sort((a, b) => {
      const dateA = new Date(a.cancellation_date).getTime();
      const dateB = new Date(b.cancellation_date).getTime();
      return dateB - dateA;
    });
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px]">
            ...
            >
              ...
              <option value="date">Fecha de Cancelación</option>
            </select>
          </div>
        ...
        <tbody className="divide-y divide-gray-200">
          {filteredAppointments.length > 0 ? (
            filteredAppointments.map((appointment) => (
              <tr key={appointment.cancelled_appointment_id} className="hover:bg-gray-50">
                <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {appointment.patient_full_name}
                </td>
                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                  {new Date(appointment.cancellation_date).toLocaleString('es-AR', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
            ...
            <tr>
              <td colSpan={2} className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                {searchTerm ? "No se encontraron resultados" : "No hay turnos cancelados"}
              </td>
            </tr>
          )}
        </tbody>
